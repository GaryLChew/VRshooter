...
public class Movement : MonoBehaviour
{
    ...
    public int walkSpeed = 0; //Standard movement speed.
    ...
    public float attackRange = 3.0f; //How close does the enemy need to be in order to attack?

    public float attackTime = 20f; //How frequent or fast an enemy can attack (cool down time).
    ...
    void Start()
    {
        ...
        playerData = GameObject.FindWithTag("PlayerInfo").GetComponent<PlayerData>();
    ...
    void Move()
    {
        ...
        if (!isDead && !isPunching)
        {
            // Set the movement vector based on the axis input.
            movement.Set(0f, 0f, -1f);

            // Normalise the movement vector and make it proportional to the speed per second.
            movement = movement.normalized * 4f * Time.deltaTime;
    ...
    void setPunch()
    {
        print("Punching!: "+attackRange+"|"+ Vector3.Distance(transform.position, target.position));
        isPunching = true;
    }
    void stopWalking()
    {
